<template>
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="flex items-center justify-center w-full md:w-auto " style="padding-top: 75px; padding-bottom: 0px; padding-left: 20px; padding-right: 20px;">
            ...
            <button>
              <img src="@/assets/images/icon/search.png" class="w-6 h-6" style="padding-top: 0px; padding-bottom:0px; padding-left: 0px; padding-right: 0px;" />
            </button>

            <div class="pb-1 text-center flex items-center" style="padding-right: 8px;">
              <button @click="toggleFavoritesFilter" 
                class="rounded-full bg-gray-200 cursor-pointer"
                ...
                <span class="text-xs pl-1 pr-4 text-gray-800 font-semibold" style="padding-right: 25px;">Favorite Activities</span>
              </button>
            </div>

            <div class="pb-1 text-center flex items-center" style="padding-right: 4px;">
              <button @click="toggleMyActivitiesFilter" 
                class="rounded-full bg-gray-200 cursor-pointer"
            ...
            <div class="pb-1 text-center flex items-center" style="padding-right: 4px;">
              <button @click="toggleMyCreationFilter" 
                class="rounded-full bg-gray-200 cursor-pointer"
          ...
          >
            <div @click="handleImageClick(activity.id)">
              <nuxt-link :to="`/activity/${activity.id}`">
                <div>
                  <img
                    v-if="activity.post_image_path"
                    :src="`http://localhost/${activity.post_image_path}`"
                    alt="Card Image"
                  ...
                  <div class="p-4">
                    ...
                    <p class="text-sm text-gray-400 mt-1.5 line-clamp-2">
                      {{ activity.detail }}
                    </p>
                  </div>
                </div>
              ...
              <div class="flex items-center px-3 mt-1">
                <div class="pb-4 text-center flex items-center" style="padding-right: 4px;">
                  <button @click="isliked(activity) ? unlikePost(activity.id) : likePost(activity.id)" 
                      class="rounded-full bg-gray-200 cursor-pointer"
                      :title="isliked(activity) ? 'Unlike' : 'Like'">
                  ...
                  <span class="text-xs pl-1 pr-4 text-gray-800 font-semibold" style="padding-right: 15px;"> {{  activity.likes.length }}</span>

                </div>
                <div class="pb-4 text-center flex items-center" style="padding-right: 4px;">
                  <button @click="navigateTo(`/activity/${activity.id}`)"
                      class="rounded-full bg-gray-200 cursor-pointer"
                      title="Comment">
                  ...
                  <span class="text-xs pl-1 pr-4 text-gray-800 font-semibold" style="padding-right: 35px;">{{  activity.comments.length }}</span>

                </div>
                <div class="pb-4 text-center flex items-center" style="padding-right: 4px;">
                  <button @click="isSaved(activity) ? unSavedPost(activity.id) : savedPost(activity.id)" 
                      class="rounded-full bg-gray-200 cursor-pointer"
                      :title="isSaved(activity) ? 'Unsaved' : 'Saved'">
                    <Icon name="bx:bxs-bookmark-star" size="45"   :color="isSaved(activity) ? '#FFD700' : ''" 
                    style="padding-top: 0px; padding-bottom: 2px; padding-left: 6px; padding-right: 6px;" />
                  </button>

                  <span class="text-xs pl-1 pr-4 text-gray-800 font-semibold" style="padding-right: 0px;">{{  activity.favorites.length }}</span>
